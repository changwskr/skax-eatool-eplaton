 ...
 */
@Controller
@RequestMapping("/workflow-generator")
public class WorkflowGeneratorController {
    
    private final NewIKesaLogger logger = NewKesaLogHelper.getLogger(this.getClass());
    
    /**
     * Workflow 생성기 페이지를 표시합니다.
     *
     * @param model 모델
     * @return 뷰 이름
     */
    @GetMapping
    public String showWorkflowGeneratorPage(Model model) {
        String className = this.getClass().getSimpleName();
        logger.info(className, "Workflow 생성기 페이지 요청");
        
        model.addAttribute("title", "Workflow 생성기");
