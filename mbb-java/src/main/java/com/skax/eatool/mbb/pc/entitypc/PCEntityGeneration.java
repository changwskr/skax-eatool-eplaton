...
public class PCEntityGeneration {
    ...
    public NewKBData getAvailableTables() {
        ...
        logger.info(className, "테이블 목록 조회 PC 시작");
        ...
        try {
            ...
            logger.info(className, "테이블 목록 조회 PC 완료: " + tables.size() + "개 테이블");
            
        } catch (Exception e) {
            response.setValid(false);
            response.setErrorMessage("테이블 목록 조회 실패: " + e.getMessage());
            
            logger.error(className, "테이블 목록 조회 PC 중 오류 발생: " + e.getMessage(), e);
        }
    ...
    public NewKBData getTableColumns(String tableName) {
        String className = this.getClass().getSimpleName();
        logger.info(className, "테이블 컬럼 정보 조회 PC 시작: " + tableName);
        ...
        try {
            ...
            logger.info(className, "테이블 컬럼 정보 조회 PC 완료: " + tableName + " (" + columns.size() + "개 컬럼)");
            
        } catch (Exception e) {
            response.setValid(false);
            response.setErrorMessage("테이블 컬럼 정보 조회 실패: " + e.getMessage());
            
            logger.error(className, "테이블 컬럼 정보 조회 PC 중 오류 발생: " + e.getMessage(), e);
        }
     ...
     * @return Entity 생성 결과
     */
    public NewKBData generateEntityFromTable(String tableName) {
        String className = this.getClass().getSimpleName();
        logger.info(className, "Entity 생성 PC 시작: " + tableName);
        
        NewKBData response = new NewKBData();
        
        try {
            EntityGenerationDDTO result = asEntityGeneration.generateEntityFromTable(tableName);
            ...
            if (result.isSuccess()) {
                ...
                logger.info(className, "Entity 생성 PC 완료: " + tableName);
            } else {
                response.setValid(false);
                response.setErrorMessage("Entity 생성 실패: " + result.getMessage());
                
                logger.error(className, "Entity 생성 PC 실패: " + tableName + " - " + result.getMessage());
            }
            ...
            logger.error(className, "Entity 생성 PC 중 오류 발생: " + e.getMessage(), e);
        }
    ...
    public NewKBData generateEntityWithColumns(Map<String, Object> request) {
        ...
        logger.info(className, "Entity 생성 PC 시작 (컬럼 정보 포함): " + tableName);
        ...
        try {
            ...
            if (result.isSuccess()) {
                ...
                logger.info(className, "Entity 생성 PC 완료 (컬럼 정보 포함): " + tableName);
            } else {
                response.setValid(false);
                response.setErrorMessage("Entity 생성 실패: " + result.getMessage());

                logger.error(className, "Entity 생성 PC 실패 (컬럼 정보 포함): " + tableName + " - " + result.getMessage());
            }
